...
const todoTemplate = document
  ...
  ?.content.querySelector(".todo-item"); //null
...
function createTask(taskName) {
  ...
  deletedTaskButton.addEventListener("click", (e) => {
    ...
    if (access) {
      ...
      const oldData = getLocalStorage("todos");
      console.log(oldData);
      const newData = oldData.filter((item) => {
        return item !== taskName;
      });
      setLocalStorage("todos", newData);
      // console.log(newData);
    }
  });
  ...
  editedTaskButton.addEventListener("click", (event) => {
    ...
    function editTodo(e) {
      ...
      event.target
        .closest(".todo-item")
        .querySelector(".todo-item__text").textContent = input.value;
...
todoForm.addEventListener("submit", addTodo);
let localData;
if (getLocalStorage("todos").length) {
  localData = getLocalStorage("todos");
} else {
  localData = todos;
}

localData.forEach((text) => {
